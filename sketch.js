...
var live1, live2, live3, live4, live5, live6, live7;
var lives = 0;
var _ammo = 10;
var retryButton;
var endBar;
var gameOver;
var extraAmmo,extraAmmo2,extraAmmo3;
function preload()
{
  ...
  retryImg = loadImage("Images/Reetry.png");

  gameOverImg = loadImage("Images/GameOver1.png");

  extraAmmoImg = loadImage("Images/bullet.png");
  }
function setup() {
  ...
  live1 = createSprite(windowWidth/2 - 520, 40, 50, 50);
  live1.addImage(livesImg);
  live1.scale = 0.11;
  live2 = createSprite(windowWidth/2 - 460, 40, 50, 50);
  live2.addImage(livesImg);
  live2.scale = 0.11;
  live3 = createSprite(windowWidth/2 - 400, 40, 50, 50);
  live3.addImage(livesImg);
  live3.scale = 0.11;
  live6 = createSprite(windowWidth/2 - 340, 40, 50, 50);
  live6.addImage(livesImg);
  live6.scale = 0.11;
  live6.visible = false;
  live7 = createSprite(windowWidth/2 - 280, 40, 50, 50);
  live7.addImage(livesImg);
  live7.scale = 0.11;
  live7.visible = false;

  live4 = createSprite(windowWidth-250, windowHeight/2-200, 50, 50);
  live4.addImage(livesImg);
  live4.scale = 0.11;

  live5 = createSprite(windowWidth-50, windowHeight/2 - 50, 50, 50);
  live5.addImage(livesImg);
  live5.scale = 0.11;
  live5.visible = false;
...
*/
  ...
  invisibleGround2 = createSprite(windowWidth-190, windowHeight/2+280, 10, 100);
  ...
  spike17 = createSprite(windowWidth-1420, windowHeight/2+28);
 ...
 // block27.debug = true;
  ...
  extraAmmo = createSprite(windowWidth-830, windowHeight/2-35, 10, 5);
  extraAmmo.addImage(extraAmmoImg);
  extraAmmo.visible = false;
  extraAmmo.scale = 0.3;

  extraAmmo2 = createSprite(windowWidth-830, windowHeight/2-45, 10, 5);
  extraAmmo2.addImage(extraAmmoImg);
  extraAmmo2.visible = false;
  extraAmmo2.scale = 0.3;

  extraAmmo3 = createSprite(windowWidth-830, windowHeight/2-55, 10, 5);
  extraAmmo3.addImage(extraAmmoImg);
  extraAmmo3.visible = false;
  extraAmmo3.scale = 0.3;
 
  //endBar = createSprite(windowWidth/2, windowHeight/2, 500, 300);
  //endBar.shapeColor = 0;
  //endBar.visible = false;
  
  gameOver = createSprite(windowWidth/2, windowHeight/2-60, 150, 30);
  gameOver.addImage(gameOverImg);
  gameOver.scale = 0.65;
  gameOver.visible = false;

  retryButton = createSprite(windowWidth/2, windowHeight/2+90, 150, 50);
  retryButton.addImage(retryImg);
  retryButton.scale = 0.3;
...
if(ammoGroup.isTouching(zombie))
{
  zombie.remove();
  ammoGroup.destroyEach();
  _ammo--;
}
if(ammoGroup.isTouching(zombie2))
{
  zombie2.remove();
  ammoGroup.destroyEach();
  _ammo--;
}
if(ammoGroup.isTouching(zombie3))
{
  zombie3.remove();
  ammoGroup.destroyEach();
  _ammo--;
...
if(soldier.isTouching(spike2)||soldier.isTouching(spike3)||soldier.isTouching(spike4)||soldier.isTouching(spike5)||soldier.isTouching(spike6)||soldier.isTouching(spike7)||soldier.isTouching(spike8))
{
...
{
  soldier.x = windowWidth-860;
  soldier.y = windowHeight/2-110;
  lives++; 
}

if(soldier.isTouching(spike1))
{
  soldier.x = windowWidth-860;
  soldier.y = windowHeight/2-110;
  lives++; 
}
if(soldier.isTouching(zombie2))
{
  soldier.x = windowWidth-860;
  soldier.y = windowHeight/2-110;
  lives++; 
}
if(soldier.isTouching(zombie3))
{
  soldier.x = windowWidth-860;
  soldier.y = windowHeight/2-110;
  lives++; 
...
{
live4.remove();
  if(soldier.isTouching(extraAmmo))
{
  _ammo = 10;
  extraAmmo.remove();
  extraAmmo2.remove();
...
if(soldier.isTouching(spike16))
{
  soldier.x = windowWidth-1220;
  soldier.y =  windowHeight/2-95;
  
  lives++;
}
if(soldier.isTouching(spike17))
{
  soldier.y = soldier.y +150;
  lives++;
}
if(soldier.isTouching(spike11))
{
  soldier.x = 150;
  lives++;
}
if(soldier.isTouching(spike13))
{
  soldier.x = 150;
  soldier.y = 660;
  lives++;
}
if(soldier.isTouching(zombie4))
{
  soldier.x = 150;
  soldier.y = 660;
  lives++;
}
if(soldier.isTouching(zombie5))
{
  soldier.x = windowWidth-830;
  soldier.y =  windowHeight/2-10;
  lives++;
}
if(soldier.isTouching(zombie6))
{
  soldier.x = windowWidth-1220;
  soldier.y =  windowHeight/2-95;
  lives++;
}
if(soldier.isTouching(spike9))
{
  soldier.x = windowWidth-500;
  soldier.y = windowHeight/2+ 230;
  lives++;
}
if(soldier.isTouching(spike10))
{
  soldier.x = windowWidth-500;
  soldier.y = windowHeight/2+ 230;
  lives++;
}
if(soldier.isTouching(spike18))
{
  soldier.x = windowWidth-500;
  soldier.y = windowHeight/2+ 230;
  lives++;
}
if(soldier.isTouching(zombie7)||soldier.isTouching(spike14)||soldier.isTouching(spike15))
{
  soldier.x = windowWidth-790;
  soldier.y = windowHeight/2 - 205;
  lives++;
}
if(soldier.isTouching(live5))
{
  lives--;
  live5.remove();
}
...
if(ammoGroup.isTouching(zombie4))
{
  zombie4.remove();
  ammoGroup.destroyEach();
  _ammo--;
}
if(ammoGroup.isTouching(zombie5))
{
  zombie5.remove();
  ammoGroup.destroyEach();
  _ammo--;
}
if(ammoGroup.isTouching(zombie6))
{
  zombie6.remove();
  ammoGroup.destroyEach();
  _ammo--;
}
if(ammoGroup.isTouching(zombie7))
{
  zombie7.remove();
  ammoGroup.destroyEach();
  _ammo--;
...
if(keyWentUp("j"))
{
  ...
  if(gameState === 1)
  {
   ...
   extraAmmo.visible = true;
   extraAmmo2.visible = true;
   extraAmmo3.visible = true;
...
fill("yellow")
  textSize(30);
  text("Ammo: "+ _ammo, windowWidth/2, windowHeight/2-325); 
}
